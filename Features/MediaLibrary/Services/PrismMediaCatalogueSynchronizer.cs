        ...
        try
        {
            ...
            foreach (var row in visitPhotos)
            {
                ...
                Upsert(existing, contentChanges, source.Id, scanId, now, new AssetValues(
                    ...
                    $"visit:{row.VisitId}",
                    $"visit:{row.VisitId}",
                    MediaCollectionTitleFormatter.FormatVisitTitle(row.VisitorName),
                    string.IsNullOrWhiteSpace(row.VisitType) ? "Visit to SDD" : row.VisitType,
                    "Visit",
                    string.IsNullOrWhiteSpace(row.Caption) ? MediaCollectionTitleFormatter.FormatVisitTitle(row.VisitorName) : row.Caption,
                    row.Caption,
                    null,
                    date,
                    row.Width,
