...
public sealed class OnnxFaceAnalysisEngine : IFaceAnalysisEngine, IDisposable
{
    ...
    public async Task<IReadOnlyList<DetectedFaceData>> AnalyseAsync(byte[] imageBytes,CancellationToken ct)
    {
        ...
        var landmarks=results.FirstOrDefault(x=>x.Name==d.LandmarksOutputName)?.AsTensor<float>(); var count=boxes.Dimensions[boxes.Dimensions.Length - 2]; var list=new List<DetectedFaceData>();
    ...
    private static float ReadScore(Tensor<float> t,int i)=>t.Dimensions.Length==1?t[i]:t.Dimensions.Length==2?t[0,i]:t[0,i,0];
    private static float[] ReadRow(Tensor<float> t,int i,int width){var a=new float[width];for(var j=0;j<width;j++)a[j]=t.Dimensions.Length==2?t[i,j]:t[0,i,j];return a;}
