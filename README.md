...
2. **Install dependencies**:
   ...
   npm install
   ```
3. **Database**: update `appsettings.Development.json` or export `DefaultConnection`. The app automatically applies migrations on startup, but if you are pointing at an existing database run `dotnet ef database update` (or `Update-Database` in Package Manager Console) to ensure tables such as `ProjectTotRequests` are created and the `__EFMigrationsHistory` table lists the latest entries.【F:Program.cs†L229-L244】  When adding new migrations, stick to PascalCase names (for example `TotTracker`) so the generated class stays within C# naming conventions and avoids the CS8981 warning about future reserved identifiers. It is safe to delete the generated migration files and create a new baseline later—the EF Core snapshot lives in the project and will re-emit the `ProjectTot`/`ProjectTotRequest` tables as long as the corresponding model types remain unchanged—just remember to reapply the fresh migrations to your databases after regenerating them.
4. **Run**:
   ```bash
