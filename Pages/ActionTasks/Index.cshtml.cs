...
{
    private static readonly Regex AutoGeneratedSprintNamePattern = new(@"^Sprint\s+(?<range>\d{1,2}\s+[A-Za-z]{3}(?:\s+\d{4})?\s*-\s*\d{1,2}\s+[A-Za-z]{3}(?:\s+\d{4})?)$", RegexOptions.IgnoreCase | RegexOptions.CultureInvariant);
    ...
    public string ResolveSprintName(int? sprintId)
    {
        ...
        return sprint is null ? $"Planning Window #{sprintId.Value}" : DisplayPlanningWindowName(sprint);
    }

    public string DisplayPlanningWindowName(ActionSprint? sprint)
    {
        // SECTION: Clean only known auto-generated legacy sprint labels while preserving user-authored names.
        if (sprint is null)
        {
