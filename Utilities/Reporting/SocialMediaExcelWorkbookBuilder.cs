...
public interface ISocialMediaExcelWorkbookBuilder
{
    byte[] Build(SocialMediaExcelWorkbookContext context);
}

public sealed record SocialMediaExcelWorkbookContext(
    IReadOnlyList<SocialMediaEventExportRow> Rows,
    DateTimeOffset GeneratedAtUtc,
    DateOnly? StartDate,
    DateOnly? EndDate,
    string? PlatformFilter);

public sealed class SocialMediaExcelWorkbookBuilder : ISocialMediaExcelWorkbookBuilder
{
    public byte[] Build(SocialMediaExcelWorkbookContext context)
    {
        ...
        var worksheet = workbook.Worksheets.Add("Social Media Events");
    ...
    private static void WriteRows(IXLWorksheet worksheet, IReadOnlyList<SocialMediaEventExportRow> rows)
    {
        var rowNumber = 2;
        ...
        for (var index = 0; index < rows.Count; index++, rowNumber++)
        {
            ...
            worksheet.Cell(rowNumber, 2).Value = row.DateOfEvent.ToDateTime(TimeOnly.MinValue);
    ...
    private static void ApplyMetadata(IXLWorksheet worksheet, SocialMediaExcelWorkbookContext context)
    {
        ...
        worksheet.Cell(metadataRow + 3, 1).Value = "Platform filter";
        worksheet.Cell(metadataRow + 3, 2).Value = context.PlatformFilter ?? "(all platforms)";
