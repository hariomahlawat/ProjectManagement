...
public sealed class GlobalProjectReportsSearchService : IGlobalProjectReportsSearchService
{
    ...
    private async Task AppendTotAsync(string pattern, int limit, ICollection<GlobalSearchHit> hits, CancellationToken cancellationToken)
    {
        var tots = await _dbContext.ProjectTots
            .AsNoTracking()
            .Where(tot => tot.Project != null && !tot.Project.IsDeleted && !tot.Project.IsArchived && (
                EF.Functions.ILike(tot.Project.Name, pattern) ||
                EF.Functions.ILike(tot.MetDetails ?? string.Empty, pattern)))
            .OrderByDescending(tot => tot.LastApprovedOnUtc ?? ToDateTime(tot.CompletedOn) ?? ToDateTime(tot.StartedOn) ?? DateTime.MinValue)
            .Take(limit)
            .Select(tot => new
            {
                tot.ProjectId,
                ProjectName = tot.Project!.Name,
                tot.MetDetails,
                tot.Status,
                tot.StartedOn,
                tot.CompletedOn,
        ...
        {
            var title = $"{tot.ProjectName} · {tot.Status}";
            var approvedDate = tot.LastApprovedOnUtc.HasValue
                ? DateTime.SpecifyKind(tot.LastApprovedOnUtc.Value, DateTimeKind.Utc)
                : (DateTime?)null;

            var date = approvedDate.HasValue
                ? new DateTimeOffset(approvedDate.Value)
                : ToDateTimeOffset(tot.CompletedOn) ?? ToDateTimeOffset(tot.StartedOn);
