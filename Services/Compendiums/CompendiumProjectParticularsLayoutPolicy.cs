...
{
    public const float FullWidthPoints = 519f;
        ...
        CompendiumDossierTextMeasurementService.Session measurementSession)
    {
        ...
        var itemWidth = Math.Max(70f, (FullWidthPoints - 20f) / columns);
        var textWidth = Math.Max(42f, itemWidth - 37f);
        ...
        foreach (var row in modules.Chunk(columns))
        {
            ...
            {
                var label = measurementSession.MeasureAtFontSize(
                    module.Label.ToUpperInvariant(), textWidth, labelSize, 1.08f).HeightPoints;
                var value = measurementSession.MeasureAtFontSize(
                    module.Value, textWidth, valueSize, 1.08f).HeightPoints;
    ...
    {
        var itemWidth = FullWidthPoints / columns;
        var textWidth = Math.Max(44f, itemWidth - 30f);
        ...
        foreach (var row in modules.Chunk(columns))
        {
            var rowHeight = row.Max(module =>
            {
                ...
                var value = measurementSession.MeasureAtFontSize(
                    module.Value, textWidth, valueSize, 1.08f).HeightPoints;
                return Math.Max(18f, label + 1.6f + value);
            });
            rowHeights.Add(rowHeight);
        }

        const float headingAndRule = 9f;
        const float headingGap = 7f;
        const float rowGap = 8f;
        const float bottomBreathingRoom = 3f;
        return headingAndRule
               + headingGap
               + rowHeights.Sum()
               + Math.Max(0, rowHeights.Count - 1) * rowGap
               + bottomBreathingRoom;
    }
