...
public sealed class DeleteModel : PageModel
{
    ...
    public DeleteModel(IMasterDataAdministrationQueryService query, IAdminMasterDataCommandService commands)
    {
    ...
    [BindProperty] public string RowVersion { get; set; } = string.Empty;
    [BindProperty] public string Confirmation { get; set; } = string.Empty;
    public bool CanDelete => Impact is { DirectUsageCount: 0, DirectChildCount: 0 };

    public async Task<IActionResult> OnGetAsync(int id, CancellationToken cancellationToken)
    {
        Impact = await _query.GetCategoryImpactAsync(MasterDataCategoryKind.Technical, id, cancellationToken);
