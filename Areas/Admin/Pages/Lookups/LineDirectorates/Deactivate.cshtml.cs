...
public sealed class DeactivateModel : PageModel
{
    ...
    {
        var result = await _commands.SetLineDirectorateActiveAsync(Id, Restore, RowVersion, cancellationToken);
        if (!result.Succeeded)
        {
            ...
            ModelState.AddModelError(string.Empty, result.UserMessage ?? "The line directorate status could not be changed.");
            return Page();
        }
    ...
    private async Task<ProjectManagement.Models.LineDirectorate?> LoadAsync(CancellationToken cancellationToken)
    {
        ...
        ProjectCount = await _db.Projects.CountAsync(project => project.SponsoringLineDirectorateId == Id, cancellationToken);
        return entity;
    }
}
