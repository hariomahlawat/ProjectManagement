...
Write-Host "PRISM Publications Phase 26 - Publication workspace and editorial structure validation"
...
foreach ($path in $required) {
    if (-not (Test-Path (Join-Path $root $path))) {
        throw "Missing Phase 26 file: $path"
    }
}
...
Write-Host "Phase 26 validation complete."
