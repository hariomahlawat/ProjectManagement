...
{
    private readonly string _root = Path.Combine(Path.GetTempPath(), "prism-activity-storage-tests", Guid.NewGuid().ToString("N"));
    ...
    public async Task OpenReadAsync_RejectsTraversalOutsideActivitiesRoot()
    {
        ...
        await File.WriteAllTextAsync(Path.Combine(_root, "other-module", "private.txt"), "private");

        var storage = new FileSystemActivityAttachmentStorage(
            new TestUploadRootProvider(_root),
