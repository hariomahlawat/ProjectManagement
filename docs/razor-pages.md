...
## Calendar
* `Pages/Calendar/Index.cshtml` – planner powered by FullCalendar for viewing organisation events. Editors can create, drag or resize entries through an offcanvas form.
* `wwwroot/js/calendar-page.js` – handles CRUD via `/calendar/events`, converts times to local inputs, auto-scrolls week views to 08:00, preserves descriptions on move/resize and blocks end-before-start submissions. Events can also be pushed to **My Tasks**.
