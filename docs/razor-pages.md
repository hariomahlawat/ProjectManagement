...
## Tasks
* `Pages/Tasks/Index.cshtml` – full management interface with filter tabs, search, page size selection, pagination, batch mark-done/delete actions, quick add and per-row inline edits for title, notes, priority, due date, pinning and mark done. Controls use small variants for a tidy table, and rows can be reordered by drag-and-drop.
