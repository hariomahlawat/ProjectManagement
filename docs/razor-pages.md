...
## Dashboard
* `Pages/Dashboard/Index.cshtml` – landing page after sign-in. It shows an attention band when tasks are overdue or due today, renders the **My Tasks** widget inline, and provides a toggle that opens the **Upcoming events** drawer on the right edge alongside placeholder content for future widgets.
