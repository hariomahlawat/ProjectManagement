...
## Calendar
* `Pages/Calendar/Index.cshtml` – organisation-wide calendar built with FullCalendar's global builds. Month, Week and List views are available. Admin, TA and HOD roles can add, edit, drag or resize events via an offcanvas form, while all signed-in users can view details and add events to their tasks. Core, DayGrid, TimeGrid and List styles are loaded explicitly for consistent visuals.
