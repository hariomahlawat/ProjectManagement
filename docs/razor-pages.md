...
## Admin analytics
* `Areas/Admin/Pages/Analytics/Logins.cshtml` – scatter chart showing login times over a selectable window with an office-hours band, percentile lines, weekend highlighting and per-user filter. Odd logins are listed below the chart and can be exported to CSV. Clicking a point opens the corresponding audit log entry.
