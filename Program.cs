...
builder.Services.AddScoped<ProliferationCommandService>();
...
var proliferationApi = app.MapGroup("/api/proliferation");

proliferationApi.MapGet("/overview", async (
    HttpRequest request,
    ProliferationOverviewService overviewService,
    CancellationToken cancellationToken) =>
{
    var years = request.Query["years"].SelectMany(ParseInts).ToArray();
    var hasYears = years.Length > 0;
    var yearsCollection = hasYears ? years : null;

    var overviewRequest = new ProliferationOverviewRequest(
        yearsCollection,
        ParseDateOnly(request.Query["dateFrom"].FirstOrDefault()),
        ParseDateOnly(request.Query["dateTo"].FirstOrDefault()),
        ParseInt(request.Query["projectCategoryId"].FirstOrDefault()),
        ParseInt(request.Query["technicalCategoryId"].FirstOrDefault()),
        ParseSource(request.Query["source"].FirstOrDefault()),
        request.Query["search"].FirstOrDefault(),
        ParseInt(request.Query["page"].FirstOrDefault()) ?? 1,
        ParseInt(request.Query["pageSize"].FirstOrDefault()) ?? 25);

    var response = await overviewService.GetOverviewAsync(overviewRequest, cancellationToken);
    return Results.Ok(response);
})
    .RequireAuthorization(new AuthorizeAttribute { Policy = ProjectOfficeReportsPolicies.ViewProliferationTracker });

proliferationApi.MapPost("/yearly", async (
    ProliferationYearlyRequestModel model,
    UserManager<ApplicationUser> userManager,
    ProliferationCommandService commandService,
    HttpContext httpContext,
    CancellationToken cancellationToken) =>
{
    ...
    try
    {
        var entity = await commandService.CreateYearlyAsync(model, user, httpContext.User, cancellationToken);
        return Results.Created($"/api/proliferation/yearly/{entity.Id}", new { entity.Id });
    }
    ...
    .RequireAuthorization(new AuthorizeAttribute { Policy = ProjectOfficeReportsPolicies.SubmitProliferationTracker });

proliferationApi.MapPost("/granular", async (
    ...
    CancellationToken cancellationToken) =>
{
    ...
    try
    {
        var entity = await commandService.CreateGranularAsync(model, user, httpContext.User, cancellationToken);
        return Results.Created($"/api/proliferation/granular/{entity.Id}", new { entity.Id });
    }
    catch (ValidationException ex)
    {
        return Results.BadRequest(new { message = ex.Message });
...
proliferationApi.MapPost("/year-preference", async (
    ...
    CancellationToken cancellationToken) =>
{
    ...
    try
    {
        await commandService.SetPreferenceAsync(model, user, cancellationToken);
        return Results.Ok();
    }
    catch (ValidationException ex)
    {
...
proliferationApi.MapPost("/import/yearly", async (
    HttpContext httpContext,
    IFormFile file,
    UserManager<ApplicationUser> userManager,
    ProliferationCommandService commandService,
    CancellationToken cancellationToken) =>
{
    ...
    try
    {
        var result = await commandService.ImportYearlyAsync(file, user, httpContext.User, cancellationToken);
    ...
    .RequireAuthorization(new AuthorizeAttribute { Policy = ProjectOfficeReportsPolicies.ManageProliferationImports });

proliferationApi.MapPost("/import/granular", async (
    HttpContext httpContext,
    IFormFile file,
    UserManager<ApplicationUser> userManager,
    ProliferationCommandService commandService,
    CancellationToken cancellationToken) =>
{
    ...
    try
    {
        var result = await commandService.ImportGranularAsync(file, user, httpContext.User, cancellationToken);
        if (result.Errors.Count > 0)
        {
            return Results.BadRequest(new { errors = result.Errors });
        }
...
proliferationApi.MapGet("/export", async (
    HttpRequest request,
    UserManager<ApplicationUser> userManager,
    ...
    CancellationToken cancellationToken) =>
{
    ...
    var projectIds = request.Query["projectIds"].SelectMany(ParseInts).ToArray();
    var source = ParseSource(request.Query["source"].FirstOrDefault()) ?? ProliferationSource.Sdd;
    var yearFrom = ParseInt(request.Query["yearFrom"].FirstOrDefault()) ?? DateTime.UtcNow.Year;
    var yearTo = ParseInt(request.Query["yearTo"].FirstOrDefault()) ?? yearFrom;

    var exportRequest = new ProliferationExportRequest(projectIds, source, yearFrom, yearTo, user.Id);
    var result = await exportService.ExportAsync(exportRequest, cancellationToken);
    ...
    return Results.File(result.File.Content, result.File.ContentType, result.File.FileName);
})
    .RequireAuthorization(new AuthorizeAttribute { Policy = ProjectOfficeReportsPolicies.ViewProliferationTracker });
...
static IEnumerable<int> ParseInts(string? value)
{
    ...
    {
        if (int.TryParse(segment.Trim(), NumberStyles.Integer, CultureInfo.InvariantCulture, out var number))
        {
            yield return number;
        }
    }
...
static DateOnly? ParseDateOnly(string? value)
    => DateOnly.TryParse(value, CultureInfo.InvariantCulture, DateTimeStyles.None, out var result) ? result : null;

static int? ParseInt(string? value)
    => int.TryParse(value, NumberStyles.Integer, CultureInfo.InvariantCulture, out var result) ? result : null;

static ProliferationSource? ParseSource(string? value)
{
    if (string.IsNullOrWhiteSpace(value))
    {
        return null;
    ...
    return Enum.TryParse<ProliferationSource>(value, true, out var parsed)
        ? parsed
        : value.Trim().Replace(" ", string.Empty, StringComparison.OrdinalIgnoreCase).ToUpperInvariant() switch
        {
