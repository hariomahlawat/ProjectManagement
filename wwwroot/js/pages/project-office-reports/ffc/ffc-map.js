(function (window, document) {
    ...
    // Fixed palette – we do NOT read from PMTheme here
    function refreshMapPalette() {
        return {
            // active footprint colours (no water blue)
            fillInstalled: '#0f766e',   // teal
            fillDelivered: '#22c55e',   // green
            fillPlanned: '#f97316',     // orange

            // geography + borders
            border: '#64748b',          // slate grey
            highlightBorder: '#334155', // darker slate
            sea: '#e0f2fe',             // very light blue
            landBase: '#f8fafc',        // light off-white land
    ...
    function buildColorScale(maxValue) {
        ...
        var baseFill = mapPalette.fillInstalled || '#0f766e';
        var colors = [
        ...
        Promise.all([
            ...
            .then(function (results) {
                ...
                var mapFrame = L.rectangle([[-85, -179.9], [85, 179.9]], {
                    ...
                    weight: 1,
                    fillOpacity: 1,
                    fillColor: mapPalette.sea
                }).addTo(map);

                // Base style – this already shows a visible landmass
                function styleFeature() {
                    return {
                        weight: 1,
                        color: mapPalette.border,
                        fillColor: mapPalette.landBase,
                        fillOpacity: 0.55, // base opacity so world silhouette is clearly visible
                        dashArray: '2 2'
                    };
                }
                ...
                function applyCountryStyle(entry) {
                    ...
                    var hasAny = hasCompleted || hasPlanned;
                    var isActive = scaleValue > 0;

                    var strokeColor = hasAny ? mapPalette.highlightBorder : mapPalette.border;
                    ...
                    entry.layer.setStyle({
                        ...
                        color: strokeColor,
                        dashArray: dashArray,
                        fillColor: isActive ? scale.colorFor(scaleValue) : mapPalette.landBase,
                        // active: strong 0.9, inactive: toned-down but clearly visible 0.55
                ...
                function onEachFeature(feature, layer) {
                    ...
                    layer.on({
                        mouseover: function (event) {
                            event.target.setStyle({ weight: 1.3, color: mapPalette.highlightBorder });
                        },
