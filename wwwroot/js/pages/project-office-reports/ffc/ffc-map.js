(function (window, document) {
    ...
    function refreshMapPalette() {
        // Start from theme palette if available, then enforce our contrast choices
        var palette = {};
        if (window.PMTheme && typeof window.PMTheme.getMapPalette === 'function') {
            palette = window.PMTheme.getMapPalette() || {};
        }

        // Ensure required keys with our desired colours
        palette.fillInstalled = '#0f766e';                       // teal for installed footprint
        palette.fillDelivered = palette.fillDelivered || '#f97316';
        palette.fillPlanned = palette.fillPlanned || '#22c55e';

        // Stronger border so world silhouette is clearly visible
        palette.border = '#64748b';                              // slate-ish, darker than before
        palette.highlightBorder = palette.highlightBorder || '#0f766e';

        palette.legendBackground = palette.legendBackground || '#f8fafc';
        palette.tooltipBackground = palette.tooltipBackground || '#ffffff';
        palette.tooltipText = palette.tooltipText || '#111827';
            ...
            .then(function (results) {
                ...
                function applyCountryStyle(entry) {
                    ...
                    entry.layer.setStyle({
                        // slightly thicker borders for clearer silhouette
                        weight: 1.5,
                        color: strokeColor,
                        dashArray: dashArray,
                        fillColor: scale.colorFor(scaleValue),
                        // stronger base opacity so non-active countries form a visible world outline
                        fillOpacity: scaleValue > 0 ? 0.9 : 0.08
                    });
                }
                ...
                function onEachFeature(feature, layer) {
                    ...
                    layer.on({
                        mouseover: function (event) {
                            event.target.setStyle({ weight: 1.7, color: mapPalette.highlightBorder });
                        },
                        mouseout: function (event) {
                            ...
                            if (countryMatch) {
                                ...
                                event.target.setStyle({ weight: 1.3, color: mapPalette.border, dashArray: '2 2' });
                            }
                        }
                    });
                }
