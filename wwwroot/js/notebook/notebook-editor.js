...
export function initNotebookEditor(board, view, options = {}) {
  ...
  async function saveEditorPayload(data) { const requestPayload = { ...data, version: item.version }; assertValidVersion(requestPayload.version); return item.type === 'Checklist' ? NotebookApi.updateChecklist(item.id, requestPayload) : NotebookApi.updateContent(item.id, requestPayload); }
