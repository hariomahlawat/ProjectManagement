...
test('phase 37.3 persists optional per-project Additional Note through schema v10 and an additive migration', () => {
  assert.match(model, /SettingsSchemaVersion\s*\{\s*get;\s*set;\s*\}\s*=\s*10/);
  assert.match(model, /string\?\s+AdditionalNote/);
  assert.match(presetService, /CurrentSchemaVersion\s*=\s*10/);
  ...
  assert.match(db, /SettingsSchemaVersion\)\.HasDefaultValue\(10\)/);
  ...
  assert.match(snapshot, /SettingsSchemaVersion"\)\.ValueGeneratedOnAdd\(\).*HasDefaultValue\(10\)/);
...
test('phase 37.3 review identity binds note content and concrete media rendition identity', () => {
  assert.match(fingerprint, /compendium-review-v15-additional-note-final-hardening/);
...
test('phase 37.3 presents Project Brief as the publication default and advances final identities', () => {
  ...
  assert.match(readService, /CompendiumPdf_2026-08-16_final-editorial-v22/);
});
