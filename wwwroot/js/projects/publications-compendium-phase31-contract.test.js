...
test('phase 31 persists dossier presentation choices through schema v7 and a forward migration', () => {
  assert.match(preset, /CurrentSchemaVersion\s*=\s*7/);
  ...
  assert.match(snapshot, /HasDefaultValue\(7\)/);
...
test('phase 31 renders adaptive technical specifications rather than fixed empty boxes', () => {
  ...
  assert.match(read('Services/Compendiums/CompendiumDossierPaginationPlanner.cs'), /longest <= (?:78|125)/);
...
test('phase 31 review fingerprint invalidates approval when adaptive dossier facts or presentation changes', () => {
  assert.match(fingerprint, /compendium-review-v(?:6-adaptive-pagination|7-adaptive-composition|8-production-hardening|9-programme-iconography|10-sponsoring-line-directorate)/);
...
test('phase 31 advances the publication build identity', () => {
  assert.match(readService, /CompendiumPdf_2026-08-(?:14_adaptive-pagination-v11|15_(?:adaptive-composition-v12|production-hardening-v13|programme-iconography-v1[45]|programme-semantics-v16|programme-particulars-v17))/);
});
