...
test('phase 37 introduces narrative alignment with schema-v9 legacy-safe persistence', () => {
  ...
  assert.match(model, /SettingsSchemaVersion\s*\{\s*get;\s*set;\s*\}\s*=\s*(?:9|10)/);
  assert.match(model, /DefaultNarrativeAlignment[\s\S]*=\s*"Left"/);
  assert.match(model, /NarrativeAlignmentOverride/);
  assert.match(presetService, /CurrentSchemaVersion\s*=\s*(?:9|10)/);
...
test('phase 37 binds typography changes into review and publication identities', () => {
  assert.match(fingerprint, /compendium-review-v(?:12-professional-typesetting|13-physical-measurement|14-editorial-constraints|15-additional-note-final-hardening)/);
  assert.match(fingerprint, /input\.NarrativeAlignment\.ToString\(\)/);
  assert.match(readService, /(?:CompendiumPdf_2026-08-15_composition-hardening-v19|CompendiumPdf_2026-08-16_(?:physical-composition-v20|editorial-constraints-v21|final-editorial-v22))/);
