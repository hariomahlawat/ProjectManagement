(() => {
    ...
    const canvas = document.getElementById('iprEventTrend');
    if (!canvas) return;
    ...
    let rows = [];
    try { rows = JSON.parse(canvas.dataset.yearly || '[]'); }
    catch { rows = []; }
    ...
    new Chart(canvas, {
        ...
        data: {
            ...
            datasets: [
                {
                    label: 'Filed',
                    data: rows.map(x => x.Filed ?? x.filed ?? 0),
        ...
        options: {
            ...
            plugins: {
                legend: {
                    position: 'top',
                    align: 'start',
                    labels: { boxWidth: 11, boxHeight: 11, color: text, usePointStyle: true }
                },
